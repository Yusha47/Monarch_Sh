...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        int temp;
        cin>>temp;
        p1.push_back(make_pair(temp, i));
    }
    for (int i = 0; i < n; i++)
    {
        int temp;
        cin>>temp;
        p2.push_back(make_pair(temp, i));
    }
    for (int i = 0; i < n; i++)
    {
        int temp;
        cin>>temp;
        p3.push_back(make_pair(temp, i));
    }
    ...
    sort(all(p3));
    int ans = 0;
   if(p1[n-1].first>=p2[n-1].first && p1[n-1].first>=p3[n-1].first)
   {
    ans+=p1[n-1].first;
    ll fidx=p1[n-1].second;
